<?php
    ...
    require_once('../conf/app.php');  // Feature toggle (FITUR_LIMIT_BIAYA_RESEP, dll) — auto-load agar tidak tergantung edit manual conf.php
...
<!-- <div class="alert alert-info" style="background: #00BCD4; color: white; border: none; border-radius: 0; padding: 12px 20px; margin-bottom: 0;">
    ...
    <strong>Info:</strong> Form E-Resep untuk pasien dengan No. Rawat: <strong><?=$norawat?></strong>
</div> -->
...
<input type="hidden" id="fitur_limit_biaya" value="<?=(defined('FITUR_LIMIT_BIAYA_RESEP') && FITUR_LIMIT_BIAYA_RESEP) ? '1' : '0'?>">
<input type="hidden" id="limit_biaya_resep" value="<?=defined('LIMIT_BIAYA_RESEP') ? LIMIT_BIAYA_RESEP : 0?>">
                        ...
                        <div class="col-md-5">
                            <div class="form-group">
                                ...
                                <div style="position: relative;">
                                    ...
                                    <input type="hidden" id="tarif_obat_json" value="">
                        ...
                        <div class="form-group">
                            ...
                            <div style="position: relative;">
                                <div class="form-line">
                                    ...
                                    <input type="hidden" id="tarif_obat_racikan_json" value="">
                                </div>
...
function hitungJumlahRacikan() {
    ...
    //console.log('hitungJumlahRacikan dipanggil');
    ...
    //console.log('Dosis Obat:', dosisObat);
    //console.log('Dosis yang diberi:', dosisRacikan);
    //console.log('Jumlah Racikan:', jumlahRacikan);
    ...
    //console.log('Parsed - dObat:', dObat, 'dRacikan:', dRacikan, 'jRacikan:', jRacikan);
    ...
    if (!isNaN(dObat) && !isNaN(dRacikan) && !isNaN(jRacikan) && 
        ...
        //console.log('Hasil perhitungan:', hasilFormat);
        elJmlRacikan.value = hasilFormat;
    } else {
        //console.log('Kondisi tidak terpenuhi, field dikosongkan');
        elJmlRacikan.value = '';
    }
}
...
function setupEventListeners() {
    ...
    //console.log('Setup event listeners untuk perhitungan racikan');
    ...
    if (elDosisRacikan) {
        ...
        //console.log('Event listener ditambahkan ke dosis_racikan');
    }
    ...
    if (elJumlahRacikan) {
        ...
        //console.log('Event listener ditambahkan ke jumlah_racikan');
    }
    
    if (elDosisObat) {
        elDosisObat.addEventListener('input', hitungJumlahRacikan);
        elDosisObat.addEventListener('change', hitungJumlahRacikan);
        //console.log('Event listener ditambahkan ke dosis_obat_racikan');
    }
}
...
// Jika jQuery tersedia, tambahkan juga listener dengan jQuery
if (typeof jQuery !== 'undefined') {
    jQuery(document).ready(function($) {
        //console.log('jQuery event listeners untuk perhitungan racikan');
                ...
                <table class="table table-bordered table-hover" style="margin-bottom: 0;">
                    <thead class="bg-green">
                        <tr>
                            ...
                            <th width="33%">NAMA OBAT & HARGA</th>
                            <th width="10%" style="text-align: center;">STOK</th>
...
<!-- TOMBOL SIMPAN + ITERASI + TARIF + TOTAL -->
<div class="row" style="margin-bottom: 15px;">
    <div class="col-md-12" style="display: flex; align-items: center; flex-wrap: wrap; gap: 12px;">
        ...
        <!-- Dropdown Jenis Tarif -->
        <div style="display: flex; align-items: center; gap: 8px; padding: 6px 14px; background: #e3f2fd; border-radius: 8px; border: 1px solid #90caf9;">
            <i class="material-icons" style="font-size: 18px; color: #1565c0; vertical-align: middle;">local_offer</i>
            <label style="margin: 0; font-weight: 500; color: #1565c0; font-size: 13px; white-space: nowrap;">Tarif :</label>
            <select id="selectJenisTarif" class="form-control"
                    style="width: 160px; height: 36px; font-size: 13px; border: 1px solid #90caf9; border-radius: 6px; padding: 0 10px; background: #fff; color: #333;">
        ...
        <?php if(defined('FITUR_ITERASI_RESEP') && FITUR_ITERASI_RESEP === true): ?>
        <?php
            $is_bpjs = false;
            $cek_bpjs = mysqli_query($GLOBALS['db_conn'], 
                "SELECT kd_pj FROM reg_periksa WHERE no_rawat='".mysqli_real_escape_string($GLOBALS['db_conn'], $norawat)."' LIMIT 1"
            );
            if($row_bpjs = mysqli_fetch_assoc($cek_bpjs)){
                $is_bpjs = ($row_bpjs['kd_pj'] === 'BPJ');
            }
        ?>
        <?php if($is_bpjs): ?>
        <div id="wrapperIterasi" style="display: flex; align-items: center; gap: 10px; padding: 8px 16px; background: #e3f2fd; border-radius: 8px; border: 1px solid #90caf9;">
            <label style="margin: 0; cursor: pointer; display: flex; align-items: center; gap: 6px; font-weight: 500; color: #1565c0; font-size: 13px; white-space: nowrap;">
                <input type="checkbox" id="chkIterasi" style="width: 18px; height: 18px; cursor: pointer; accent-color: #1565c0;">
                <i class="material-icons" style="font-size: 18px; color: #1565c0;">repeat</i> Iterasi
            </label>
            <select id="selectIterasi" class="form-control" disabled
                    style="width: 150px; height: 36px; font-size: 13px; border: 1px solid #90caf9; border-radius: 6px; padding: 0 10px; background: #fff; color: #333;">
                <option value="">-- Pilih --</option>
                <option value="1. Iterasi 1x">Iterasi 1x</option>
                <option value="2. Iterasi 2x">Iterasi 2x</option>
            </select>
        </div>
        <?php endif; ?>
        <?php endif; ?>
...
<!-- BOX TOTAL BIAYA -->
<div id="wrapperBoxTotal" class="row" style="margin-top: 10px; margin-bottom: 15px; display: none;">
    <div class="col-md-12">
        <div style="display: flex; gap: 12px; flex-wrap: wrap; align-items: stretch;">
            <div style="background: #e8f5e9; border: 1px solid #a5d6a7; border-radius: 8px; padding: 10px 20px; min-width: 200px;">
                <div style="font-size: 11px; color: #555; font-weight: 600; text-transform: uppercase; margin-bottom: 4px;">
                    <i class="material-icons" style="font-size: 14px; vertical-align: middle;">local_pharmacy</i> Total Non Racikan
                </div>
                <div id="totalNonRacikanDisplay" style="font-size: 18px; font-weight: 700; color: #2e7d32;">Rp 0</div>
            </div>
            <div style="background: #fff3e0; border: 1px solid #ffcc80; border-radius: 8px; padding: 10px 20px; min-width: 200px;">
                <div style="font-size: 11px; color: #555; font-weight: 600; text-transform: uppercase; margin-bottom: 4px;">
    ...
    <div class="card-footer" style="background: #f5f5f5; padding: 15px 20px; border-radius: 0 0 12px 12px;">
        <div class="row">
            <div class="col-md-4">
                ...
                <span style="font-size: 12px; color: #999;">
                    <i class="material-icons" style="font-size: 14px; vertical-align: middle;">visibility</i>
                    Menampilkan: <span id="showingResepDB">0</span> dari <span id="totalResepDB2">0</span>
                </span>
            </div>
            <div class="col-md-4" style="text-align: center;">
                <!-- Pagination Controls -->
            ...
            <div class="col-md-4" style="text-align: right;">
                <span id="lastUpdateTime" style="color: #999; font-size: 12px; margin-right: 10px;">
...
<style>
/* CSS untuk tabel riwayat resep kompleks */
...
/* Border tebal antar resep */
#tabelRiwayatResepDB tr.resep-group-border {
    border-top: 5px solid #673ab7 !important;
}

#tabelRiwayatResepDB tr.resep-group-border td {
    border-top: 5px solid #673ab7 !important;
}

/* Border tebal khusus untuk resep pertama */
#tabelRiwayatResepDB tr.resep-group-border:first-child {
    border-top: none !important;
}

#tabelRiwayatResepDB tr.resep-group-border:first-child td {
    border-top: none !important;
}
...
<!-- SESUDAH - dengan cache buster -->
<script src="js/eresep.js?v=<?=time()?>"></script>