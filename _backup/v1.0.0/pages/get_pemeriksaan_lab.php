...
try {
    ...
    // Ambil config set_tarif — kelas_lab skip untuk ralan
    $set = mysqli_fetch_assoc(bukaquery("SELECT cara_bayar_lab FROM set_tarif LIMIT 1"));

    // Build filter kondisional
    $filter_bayar = '';

    if($set && !empty($norawat)){
        if($set['cara_bayar_lab'] === 'Yes'){
