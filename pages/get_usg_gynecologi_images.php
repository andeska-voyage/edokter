<?php
/**
 * get_usg_gynecologi_images.php
 * Handler untuk mengambil gambar USG Gynecologi dari database atau Orthanc
 * 
 * Logic:
 * 1. Check database (hasil_pemeriksaan_usg_gynecologi_gambar) dulu
...
try {
    ...
    $query_images = "SELECT photo FROM hasil_pemeriksaan_usg_gynecologi_gambar 
    ...
    if ($result_images && mysqli_num_rows($result_images) > 0) {
        while ($row = mysqli_fetch_assoc($result_images)) {
            $photo_url = USG_GYNECOLOGI_BASE_URL . $row['photo'];
    ...
    $query_patient = "SELECT 
                        h.tanggal,
                        r.no_rkm_medis
                      FROM hasil_pemeriksaan_usg_gynecologi h
    ...
    error_log("[USG-GYN-IMG] Query Orthanc: NORM=$no_rkm_medis, Date=$study_date (from: $tanggal)");
    
    $orthanc_series = $orthanc->getAllSeries($no_rkm_medis, $study_date);
    
    error_log("[USG-GYN-IMG] Series found: " . count($orthanc_series));
    ...
    error_log("[USG-GYN-IMG] Thumbnails count: " . count($thumbnails));
    ...
    error_log("[USG-GYN-IMG] ERROR: " . $e->getMessage());
    echo json_encode([
        'status' => 'error',
