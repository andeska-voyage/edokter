...
if(!function_exists('renderSOAPIETable')) {
    function renderSOAPIETable($query, $page, $total_data, $total_pages, $limit, $offset, $type = 'ralan', $filter_norawat = '') {
        ...
        foreach($grouped as $no_rawat => $rows) {
            ...
            foreach($rows as $idx => $row) {
                ...
                // Format vital signs (dengan tanggal pemeriksaan + petugas di atas + tombol Copy)
                $vitalSigns = '
                <div class="vital-signs">
                    ...
                    <div style="margin-top: 12px; padding-top: 10px; border-top: 1px dashed #ddd;">
                        <button type="button" class="btn btn-primary btn-xs waves-effect copy_soap_soapie"
                            ...
                            data-alergi="'.htmlspecialchars($row['alergi'] ?? '', ENT_QUOTES).'"
        ...
        // Gunakan filter_norawat jika ada
        $filter_js = !empty($filter_norawat) ? ', null, "'.$filter_norawat.'"' : '';
        echo '<script>
            if(typeof loadSOAPIEPage === "undefined") {
                //console.log("⚠️ loadSOAPIEPage not found, creating fallback");
                window.loadSOAPIEPage = function(page, type, scroll, filter) {
                    if(typeof SOAPIEModule !== "undefined" && SOAPIEModule.loadPage) {
                        SOAPIEModule.loadPage(page, type, scroll || false, filter || null);
                    } else {
            ...
            // Helper function dengan filter
            window.loadSOAPIEPageWithFilter = function(page, type) {
                var filterVal = document.getElementById("filter-norawat-soapie");
                var filter = filterVal ? filterVal.value : "";
                loadSOAPIEPage(page, type, false, filter);
            };
        </script>';
...
if($action == 'load_soapie_ranap'){
    $norm = isset($_GET['norm']) ? validTeks4($_GET['norm'], 20) : '';
    $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
    $current_norawat = isset($_GET['current_norawat']) ? validTeks4($_GET['current_norawat'], 20) : '';
    $filter_norawat = isset($_GET['filter_norawat']) ? validTeks4($_GET['filter_norawat'], 20) : '';
    $limit = 5;
    $offset = ($page - 1) * $limit;
    ...
    // Query untuk ambil semua no_rawat untuk dropdown filter
    $query_norawat_list = bukaquery("SELECT DISTINCT pr.no_rawat, rp.tgl_registrasi
    ...
    $norawat_options = [];
    while($row_nr = mysqli_fetch_array($query_norawat_list)) {
        $norawat_options[] = $row_nr;
    }
    
    // Build WHERE clause
    $where_filter = "rp.no_rkm_medis = '$norm'";
    if(!empty($filter_norawat)) {
        $where_filter .= " AND pr.no_rawat = '$filter_norawat'";
    }
    
    // Query untuk hitung total data RAWAT INAP (dengan filter)
    $query_count = bukaquery("SELECT COUNT(*) as total 
                              FROM pemeriksaan_ranap pr
                              LEFT JOIN reg_periksa rp ON pr.no_rawat = rp.no_rawat
                              WHERE $where_filter");
    ...
    echo '<div class="soapie-filter-header" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 12px 15px; border-radius: 8px 8px 0 0; margin-bottom: 0;">';
    echo '<div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">';
    echo '<div style="display: flex; align-items: center; gap: 10px;">';
    echo '<i class="material-icons" style="font-size: 24px;">assignment</i>';
    echo '<h5 style="margin: 0; font-weight: 600;">Riwayat SOAPIE Rawat Inap</h5>';
    echo '</div>';
    echo '<div style="display: flex; align-items: center; gap: 15px; flex-wrap: wrap;">';
    echo '<div style="display: flex; align-items: center; gap: 8px;">';
    echo '<label style="font-size: 12px; margin: 0;">Filter No. Rawat:</label>';
    echo '<select id="filter-norawat-soapie" onchange="filterSOAPIERanap(this.value)" style="padding: 5px 10px; border-radius: 4px; border: 1px solid rgba(255,255,255,0.3); background: rgba(255,255,255,0.2); color: white; font-size: 12px; min-width: 180px;">';
    echo '<option value="" style="background: #667eea; color: white;" '.($filter_norawat == '' ? 'selected' : '').'>-- Semua No. Rawat --</option>';
    foreach($norawat_options as $opt) {
        $selected = ($filter_norawat == $opt['no_rawat']) ? 'selected' : '';
        $is_current = ($opt['no_rawat'] == $current_norawat) ? ' ⭐' : '';
        echo '<option value="'.$opt['no_rawat'].'" style="background: #667eea; color: white;" '.$selected.'>'.$opt['no_rawat'].$is_current.'</option>';
    }
    echo '</select>';
    echo '</div>';
    echo '<span style="font-size: 11px; opacity: 0.9;">';
    echo '<i class="material-icons" style="font-size: 14px; vertical-align: middle;">schedule</i> ';
    echo date('d/m/Y H:i:s');
    echo '</span>';
    echo '<button type="button" onclick="SOAPIEModule.reloadRanap()" class="btn btn-sm" style="background: rgba(255,255,255,0.2); border: 1px solid rgba(255,255,255,0.3); color: white; padding: 4px 10px;">';
    echo '<i class="material-icons" style="font-size: 16px; vertical-align: middle;">refresh</i> Reload';
    echo '</button>';
    echo '</div>';
    echo '</div>';
    echo '</div>';
    ...
    if($total_data == 0){
        echo '<div class="alert alert-info" style="margin: 15px; border-radius: 0 0 8px 8px;">Tidak ada data SOAPIE Rawat Inap'.((!empty($filter_norawat)) ? ' untuk No. Rawat: '.$filter_norawat : '').'.</div>';
        
        // Script untuk filter
        echo '<script>
        function filterSOAPIERanap(norawat) {
            if(typeof SOAPIEModule !== "undefined") {
                SOAPIEModule.loadPage(1, "ranap", false, norawat);
            }
        }
        </script>';
        exit();
    ...
    // Query data RAWAT INAP (dengan filter)
    $query = bukaquery("SELECT 
                            pr.no_rawat,
                            pr.tgl_perawatan,
                            pr.jam_rawat,
    ...
    // Inline script untuk handler Copy
    echo '
    <script>
    (function() {
        var $ = jQuery;
        ...
        // Handler tombol Copy SOAPIE
        $(".copy_soap_soapie").off("click").on("click", function(e) {
            e.preventDefault();
            var btn = $(this);
            var data = btn.data();
