...
// Cek data existing di tabel penyakit dalam
$queryCheck = bukaquery("SELECT pmr.*, d.nm_dokter as nama_dokter_pengisi 
                         FROM penilaian_medis_ralan_penyakit_dalam pmr
...
// Data default sesuai tabel penilaian_medis_ralan_penyakit_dalam
$data = array(
...
<div class="modern-form-container">
    ...
    <div class="patient-header">
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:8px;">
            <h1 style="margin:0; display:flex; align-items:center; gap:10px;">
                ...
                PENILAIAN AWAL MEDIS RAWAT JALAN PENYAKIT DALAM
                <?php if ($isEdit): ?>
            ...
            <div style="display:flex; align-items:center; gap:10px; background:#f8f9fa; border-radius:8px; padding:8px 12px;">
                ...
                <div style="display:flex; flex-direction:column; gap:2px;">
                    <div style="display:flex; align-items:center; gap:5px;">
                        ...
                        <span id="progress-text-pdalam" style="font-weight:bold; font-size:14px; color:#6c757d;">0%</span>
                    </div>
                    <div style="width:150px; height:8px; background:#e9ecef; border-radius:4px; overflow:hidden;">
                        <div id="progress-bar-pdalam" style="width:0%; height:100%; transition:width 0.3s ease, background 0.3s ease;"></div>
                    </div>
                </div>
                <span id="progress-status-pdalam" style="font-size:10px; color:#6c757d; white-space:nowrap;">(0/0)</span>
            </div>
        </div>
    ...
    <div class="form-card">
        <div class="form-content">
            <form id="formPenilaianMedisPDalam" method="post" action="">
                ...
                <div class="section">
                    ...
                    <!-- Baris 1: Status Nutrisi | Kondisi Umum | GCS -->
                    <div class="form-grid cols-3">
                        <div class="form-group">
                            <label>Status Nutrisi</label>
                            <input type="text" name="status"
                                   value="<?php echo htmlspecialchars($data['status']); ?>"
                                   placeholder="Baik / Kurang / Buruk">
                        </div>
                        <div class="form-group">
                            <label>Kondisi Umum</label>
                            <input type="text" name="kondisi"
                                   value="<?php echo htmlspecialchars($data['kondisi']); ?>"
                                   placeholder="Deskripsikan kondisi umum pasien...">
                        </div>
                        <div class="form-group">
                            <label>GCS (E,V,M)</label>
                            <input type="text" name="gcs"
                                   value="<?php echo htmlspecialchars($data['gcs']); ?>" placeholder=" ">
                        </div>
                    </div>

                    <!-- Baris 2: Tanda-Tanda Vital semua berjajar -->
                    <div class="section-subtitle" style="margin-top:12px;">Tanda-Tanda Vital</div>
                    <div class="vital-grid">
                        ...
                        <div class="vital-item">
                            ...
                            <input type="text" name="nyeri"
                                   value="<?php echo htmlspecialchars($data['nyeri']); ?>" placeholder="Skala / lokasi">
                        </div>
                    </div>
                </div>
                ...
                <div class="section">
                    ...
                    $statusKelainan = [
                        ...
                        ['thoraks',     'keterangan_thorak',      'Thoraks',     'favorite'],
                    ...
                    $statusColor = [
                        ...
                        'Abnormal'        => ['bg'=>'#fee2e2','color'=>'#991b1b','border'=>'#fca5a5'],
                        'Tidak Diperiksa' => ['bg'=>'#f1f5f9','color'=>'#475569','border'=>'#cbd5e1'],
                    ];
                    ?>

                    <div style="display:flex; gap:16px; align-items:stretch;">

                        <!-- Kolom kiri: 4 baris kelainan -->
                        <div style="flex:1; display:flex; flex-direction:column; gap:6px;">
                            <?php foreach ($statusKelainan as $sk):
                                $currentVal = $data[$sk[0]];
                                $sc = isset($statusColor[$currentVal]) ? $statusColor[$currentVal] : $statusColor['Normal'];
                            ?>
                            <div style="display:flex; align-items:center; gap:0; background:#f8fafc; border:1px solid #e2e8f0; border-radius:6px; overflow:hidden; transition:box-shadow 0.2s;"
                                ...
                                <!-- Label dengan icon -->
                                <div style="display:flex; align-items:center; gap:5px; padding:0 10px; min-width:120px; background:linear-gradient(135deg,#667eea,#764ba2); color:white; height:100%; align-self:stretch; justify-content:flex-start;">
                                    <i class="material-icons" style="font-size:14px; opacity:0.85;"><?php echo $sk[3]; ?></i>
                                    <span style="font-size:11px; font-weight:700; letter-spacing:0.3px; text-transform:uppercase;"><?php echo $sk[2]; ?></span>
                                </div>

                                <!-- Select dengan badge warna -->
                                <div style="position:relative; flex-shrink:0;">
                                    <select name="<?php echo $sk[0]; ?>"
                                            style="appearance:none; -webkit-appearance:none;
                                                   padding:9px 32px 9px 12px; border:none; border-right:1px solid #e2e8f0;
                                                   font-size:12px; font-weight:600; cursor:pointer; outline:none;
                                                   background:<?php echo $sc['bg']; ?>;
                                                   color:<?php echo $sc['color']; ?>;
                                                   min-width:148px;"
                                            onchange="
                                                var colors={'Normal':{'bg':'#dcfce7','color':'#166534'},'Abnormal':{'bg':'#fee2e2','color':'#991b1b'},'Tidak Diperiksa':{'bg':'#f1f5f9','color':'#475569'}};
                                                var c=colors[this.value]||colors['Normal'];
                                                this.style.background=c.bg; this.style.color=c.color;
                                                this.nextElementSibling.style.color=c.color;
                                            ">
                                        <?php foreach ($enumNormal as $opt): ?>
                                    ...
                                    <!-- Custom arrow -->
                                    <span style="position:absolute; right:10px; top:50%; transform:translateY(-50%); pointer-events:none; color:<?php echo $sc['color']; ?>; font-size:14px;">▾</span>
                                </div>

                                <!-- Input keterangan -->
                                <input type="text" name="<?php echo $sk[1]; ?>"
                                       value="<?php echo htmlspecialchars($data[$sk[1]]); ?>"
                                       placeholder="Keterangan tambahan..."
                                       style="flex:1; border:none; padding:9px 12px; font-size:12px;
                                              background:white; outline:none; min-width:0; color:#374151;"
                        ...
                        <div style="width:280px; flex-shrink:0; display:flex; flex-direction:column;">
                            ...
                            <textarea name="lainnya"
                                      style="flex:1; resize:none; border:1px solid #e2e8f0; border-top:none;
                                             ...
                                             background:white; outline:none; min-height:130px;"
                    ...
                    <div class="form-grid cols-3">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="rad" rows="4"
                                      placeholder="Hasil pemeriksaan radiologi..."><?php echo htmlspecialchars($data['rad']); ?></textarea>
                        </div>
                        <div class="form-group">
                            <label>Penunjang Lainnya</label>
                            <textarea name="penunjanglain" rows="4"
                                      placeholder="Hasil pemeriksaan penunjang lainnya..."><?php echo htmlspecialchars($data['penunjanglain']); ?></textarea>
                        </div>
                    </div>
                </div>
        ...
        <div class="action-bar">
            <button type="button" class="btn btn-secondary" onclick="kembaliAwalMedisPDalam()">
            ...
            if ($bolehHapus): ?>
            <button type="button" id="btn-delete-pdalam" class="btn btn-danger" onclick="confirmDeletePDalam()">
            ...
            <button type="submit" id="btn-save-pdalam" form="formPenilaianMedisPDalam" class="btn btn-primary">
...
<script src="<?php echo BASE_URL; ?>/js/awalmedispenyakitdalam.js?v=<?php echo time(); ?>"></script>
