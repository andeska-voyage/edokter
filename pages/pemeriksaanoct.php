...
$queryCheck = bukaquery("SELECT * FROM hasil_pemeriksaan_oct WHERE no_rawat='$no_rawat'");
...
$images = [];
if($isEdit) {
    $qi = bukaquery("SELECT photo FROM hasil_pemeriksaan_oct_gambar WHERE no_rawat='$no_rawat'");
...
<div class="modern-form-container">
    <div class="patient-header">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:20px;">
            <div style="display:flex;align-items:center;gap:10px;flex-shrink:0;">
                <i class="material-icons" style="font-size:22px;">remove_red_eye</i>
                <h2 style="margin:0;font-size:15px;font-weight:700;white-space:nowrap;">PEMERIKSAAN OCT</h2>
            </div>
    ...
    <div class="form-wrapper">
        <div class="modern-tabs">
            <button class="tab-item active" onclick="switchTabOct(0)"><i class="material-icons">description</i> Form Pemeriksaan</button>
            <button class="tab-item" onclick="switchTabOct(1)"><i class="material-icons">image</i> Gambar OCT
                <span class="tab-badge" id="badge-oct-1" <?php echo count($images)===0?'style="display:none;"':''; ?>><?php echo count($images); ?></span>
            </button>
        </div>

        <form id="formOct" method="post" action="">
            ...
            <!-- TAB 0: FORM DATA -->
            <div class="tab-content active" id="tab-oct-0">
                <div class="section-card" style="padding:15px;">
                    ...
                    <div class="form-group-modern" style="margin-bottom:0;">
                        ...
                        <textarea class="form-control-modern" name="hasil_pemeriksaan" rows="8"
            ...
            <!-- TAB 1: GAMBAR -->
            <div class="tab-content" id="tab-oct-1">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                    <h3 style="margin:0;"><i class="material-icons">photo_library</i> Gambar OCT</h3>
                    <div>
                        <input type="file" id="manual-upload-oct" accept="image/*" multiple style="display:none;" onchange="uploadManualOct(this)">
                        <button type="button" onclick="document.getElementById('manual-upload-oct').click()"
                ...
                <div id="image-gallery-oct"><div class="loading-spinner"><i class="material-icons">info</i> Klik tab untuk memuat gambar...</div></div>
            </div>
            ...
            <div class="action-buttons">
                <div style="display:flex;align-items:center;gap:15px;">
                    ...
                    <span style="font-size:12px;color:#666;font-weight:600;">Tab <span id="current-tab-number-oct">1</span> dari 2</span>
                </div>
                <div style="display:flex;gap:10px;">
                    <button type="button" class="btn-modern btn-secondary-modern" onclick="kembaliOct()"><i class="material-icons" style="font-size:16px;">arrow_back</i> KEMBALI</button>
                    <button type="button" class="btn-modern btn-secondary-modern" id="btn-prev-oct" onclick="previousTabOct()" style="display:none;"><i class="material-icons" style="font-size:16px;">navigate_before</i> SEBELUMNYA</button>
                    <button type="button" class="btn-modern btn-primary-modern" id="btn-next-oct" onclick="nextTabOct()">SELANJUTNYA <i class="material-icons" style="font-size:16px;">navigate_next</i></button>
                    <button type="submit" class="btn-modern btn-primary-modern" id="btn-save-oct"><i class="material-icons" style="font-size:16px;">save</i> SIMPAN DATA</button>
                    <button type="button" class="btn-modern btn-danger-modern" id="btn-delete-oct" onclick="confirmDeleteOct()" <?php echo !$isEdit?'disabled':''; ?>><i class="material-icons" style="font-size:16px;">delete</i> HAPUS DATA</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <!-- Modal Gambar -->
    <div id="imageModalOct" style="display:none;position:fixed;z-index:9999;left:0;top:0;width:100%;height:100%;background:rgba(0,0,0,0.9);" onclick="closeImageModalOct()">
        <span style="position:absolute;top:15px;right:35px;color:#f1f1f1;font-size:40px;font-weight:bold;cursor:pointer;">&times;</span>
        <img id="modalImageOct" style="margin:auto;display:block;max-width:90%;max-height:90%;object-fit:contain;margin-top:50px;">
    </div>
</div>
...
<script>
(function(){
    if(window._octJsLoaded){if(typeof window._octInit==='function')setTimeout(window._octInit,300);return;}
    var s=document.createElement('script');s.src='<?php echo BASE_URL; ?>/js/pemeriksaanoct.js?v=<?php echo time(); ?>';
    s.onload=function(){window._octJsLoaded=true;};document.head.appendChild(s);
})();
</script>