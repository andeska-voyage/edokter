...
try {
    // STEP 1: DATABASE
    $result_images = bukaquery("SELECT photo FROM hasil_pemeriksaan_usg_neonatus_gambar WHERE no_rawat='$no_rawat_safe' ORDER BY photo ASC");
    $db_images = [];
    if ($result_images && mysqli_num_rows($result_images) > 0) {
        while ($row = mysqli_fetch_assoc($result_images)) {
            $db_images[] = ['source'=>'database','data'=> USG_NEONATUS_BASE_URL . $row['photo'],'type'=>'image','photo_path'=>$row['photo']];
        }
    }
    if (!empty($db_images)) {
    ...
    // STEP 2: ORTHANC
    $result_patient = bukaquery("SELECT h.tanggal, r.no_rkm_medis FROM hasil_pemeriksaan_usg_neonatus h INNER JOIN reg_periksa r ON h.no_rawat=r.no_rawat WHERE h.no_rawat='$no_rawat_safe' LIMIT 1");
    ...
    error_log("[USG-NEO-IMG] ERROR: " . $e->getMessage());
