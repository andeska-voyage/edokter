...
$queryCheck = bukaquery("SELECT * FROM hasil_pemeriksaan_usg_gynecologi WHERE no_rawat = '$no_rawat'");
...
$images = array();
if($isEdit) {
    $queryImages = bukaquery("SELECT photo FROM hasil_pemeriksaan_usg_gynecologi_gambar WHERE no_rawat = '$no_rawat'");
...
<div class="modern-form-container">
    ...
    <div class="patient-header">
        <div style="display: flex; justify-content: space-between; align-items: center; gap: 20px;">
            ...
            <div style="display: flex; align-items: center; gap: 10px; flex-shrink: 0;">
                <i class="material-icons" style="font-size: 22px;">medical_services</i>
                <h2 style="margin: 0; font-size: 15px; font-weight: 700; white-space: nowrap;">
                    PEMERIKSAAN USG GYNECOLOGI
                </h2>
            </div>
    ...
    <div class="form-wrapper">
        ...
        <div class="modern-tabs">
            <button class="tab-item active" onclick="switchTabUSGGyn(0)">
                <i class="material-icons">description</i> Form Pemeriksaan
                <span class="tab-badge" id="badge-usggyn-0" style="display:none;"></span>
            </button>
            <button class="tab-item" onclick="switchTabUSGGyn(1)">
                <i class="material-icons">image</i> Gambar USG
                <span class="tab-badge" id="badge-usggyn-1" style="display:none;"><?php echo count($images); ?></span>
            </button>
        </div>

        <!-- Form with scroll wrapper -->
        <form id="formUSGGyn" method="post" action="">
        ...
        <div class="tab-content active" id="tab-usggyn-0">
            <div class="section-card" style="padding: 15px;">
                ...
                <div class="form-group-modern" style="margin-bottom: 10px;">
                    <label style="font-size: 11px; margin-bottom: 3px;">Uterus</label>
                    <textarea class="form-control-modern" name="uterus" rows="3" style="padding: 6px 10px; font-size: 12px;"><?php echo $data['uterus']; ?></textarea>
                </div>

                <!-- Row 4: Parametrium -->
                <div class="form-group-modern" style="margin-bottom: 10px;">
                    <label style="font-size: 11px; margin-bottom: 3px;">Parametrium</label>
                    <textarea class="form-control-modern" name="parametrium" rows="3" style="padding: 6px 10px; font-size: 12px;"><?php echo $data['parametrium']; ?></textarea>
                </div>

                <!-- Row 5: Ovarium -->
                <div class="form-group-modern" style="margin-bottom: 10px;">
                    <label style="font-size: 11px; margin-bottom: 3px;">Ovarium</label>
                    <textarea class="form-control-modern" name="ovarium" rows="3" style="padding: 6px 10px; font-size: 12px;"><?php echo $data['ovarium']; ?></textarea>
                </div>

                <!-- Row 6: Doppler -->
                <div class="form-group-modern" style="margin-bottom: 10px;">
                    <label style="font-size: 11px; margin-bottom: 3px;">Doppler</label>
                    <textarea class="form-control-modern" name="doppler" rows="3" style="padding: 6px 10px; font-size: 12px;"><?php echo $data['doppler']; ?></textarea>
                </div>
        ...
        <!-- TAB 1: GAMBAR USG -->
        <div class="tab-content" id="tab-usggyn-1">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                <h3 style="margin:0;"><i class="material-icons">photo_library</i> Gambar USG Gynecologi</h3>
                <div>
                    <input type="file" id="manual-upload-usggyn" accept="image/*" multiple style="display:none;" onchange="uploadManualUSGGyn(this)">
                    <button type="button" onclick="document.getElementById('manual-upload-usggyn').click()"
            ...
            <div id="image-gallery-usggyn"><div class="loading-spinner"><i class="material-icons">info</i> Klik tab untuk memuat gambar...</div></div>
        </div><!-- End tab-content -->
        ...
        <div class="action-buttons">
            <div style="display: flex; align-items: center; gap: 15px;">
                <div class="progress-indicator">
                    <div class="progress-dot" id="dot-usggyn-0"></div>
                    <div class="progress-dot" id="dot-usggyn-1"></div>
                </div>
                <span style="font-size: 12px; color: #666; font-weight: 600;">
                    Tab <span id="current-tab-number-usggyn">1</span> dari 2
                </span>
            </div>
            <div style="display: flex; gap: 10px;">
                <button type="button" class="btn-modern btn-secondary-modern" onclick="kembaliUSGGyn()">
                    <i class="material-icons" style="font-size: 16px;">arrow_back</i>
                    KEMBALI
                </button>
                <button type="button" class="btn-modern btn-secondary-modern" id="btn-prev-usggyn" onclick="previousTabUSGGyn()" style="display: none;">
                    <i class="material-icons" style="font-size: 16px;">navigate_before</i>
                    SEBELUMNYA
                </button>
                <button type="button" class="btn-modern btn-primary-modern" id="btn-next-usggyn" onclick="nextTabUSGGyn()">
                    SELANJUTNYA
                    <i class="material-icons" style="font-size: 16px;">navigate_next</i>
                </button>
                <button type="submit" name="btnSimpan" class="btn-modern btn-primary-modern" id="btn-save-usggyn" style="display: none;">
                    <i class="material-icons" style="font-size: 16px;">save</i>
                    SIMPAN DATA
                </button>
                <button type="button" name="btnHapus" class="btn-modern btn-danger-modern" id="btn-delete-usggyn" style="display: none;" 
                        onclick="confirmDeleteUSGGyn()" <?php echo !$isEdit ? 'disabled' : ''; ?>>
    ...
    <!-- Image Modal -->
    <div id="imageModalUSGGyn" style="display: none; position: fixed; z-index: 9999; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.9);" onclick="closeImageModalUSGGyn()">
        <span style="position: absolute; top: 15px; right: 35px; color: #f1f1f1; font-size: 40px; font-weight: bold; cursor: pointer;">&times;</span>
        <img id="modalImageUSGGyn" style="margin: auto; display: block; max-width: 90%; max-height: 90%; object-fit: contain; margin-top: 50px;">
    </div>
</div>
...
<script>
(function() {
    if (window._usgGynJsLoaded) {
        console.log('[USG-GYN] JS already loaded, re-initializing...');
        if (typeof window._usgGynInit === 'function') {
            setTimeout(window._usgGynInit, 300);
        }
        return;
    }
    var script = document.createElement('script');
    script.src = '<?php echo BASE_URL; ?>/js/pemeriksaanusggynecologi.js?v=<?php echo time(); ?>';
    script.onload = function() {
        console.log('[USG-GYN] JS loaded successfully via dynamic script');
        window._usgGynJsLoaded = true;
    };
    script.onerror = function() {
        console.error('[USG-GYN] Failed to load JS file');
    };
