...
if($action == 'load_riwayat'){
    ...
    // Query data riwayat
    $query_periksa = bukaquery("SELECT pr.*, pg.nama as nama_petugas, pl.nm_poli
    ...
    $html .= '<table class="table table-bordered no-padding riwayat-table" width="100%">
                <thead>
                    <tr>
                        <th width="30">NO</th>
                        <th width="280">TANGGAL & VITAL SIGNS</th>
                        <th>SOAPIE</th>
                        <th width="80">AKSI</th>
                    </tr>
                </thead>
        ...
        while($value = mysqli_fetch_array($query_periksa)){
            ...
            $vitalSigns = '
                <div style="line-height: 1.8;">
                    ...
                    <div><span class="vital-label">Lingkar Perut</span>: '.$value['lingkar_perut'].' cm</div>
                </div>
            ';
            ...
            $html .= '<tr>
                        ...
                        <td>
                            <strong style="color: #333;">'.konversiTanggal($value['tgl_perawatan']).'</strong> <span style="color: #666;">'.$value['jam_rawat'].'</span><br>
                            <small style="color: #999;">'.($value['nm_poli'] ? $value['nm_poli'] : 'Poliklinik Umum').'</small><br>
...
if($action == 'load_pemeriksaan'){
    ...
    $query = bukaquery("SELECT pr.*, pg.nama as nama_petugas, pl.nm_poli
                        FROM pemeriksaan_ralan pr
                        LEFT JOIN pegawai pg ON pr.nip = pg.nik
                        LEFT JOIN reg_periksa rp ON pr.no_rawat = rp.no_rawat
                        LEFT JOIN poliklinik pl ON rp.kd_poli = pl.kd_poli
    ...
    while($row = mysqli_fetch_array($query)){
        ...
        echo '<tr>
                ...
                <td>
                    ...
                    <small style="color: #999;">'.$row['nm_poli'].'</small><br>
        ...
        if($is_owner) {
            ...
            echo '<button class="btn btn-xs btn-warning edit_pemeriksaan" 
                            ...
                            data-lingkar_perut="'.$row['lingkar_perut'].'"
    ...
    (function() {
        ...
        //console.log("✅ Tabel pemeriksaan loaded - " + document.querySelectorAll(".edit_pemeriksaan").length + " tombol Edit");
        //console.log("✅ Tabel pemeriksaan loaded - " + document.querySelectorAll(".delete_pemeriksaan").length + " tombol Delete");
        //console.log("✅ Tabel pemeriksaan loaded - " + document.querySelectorAll(".copy_soap").length + " tombol Copy");
