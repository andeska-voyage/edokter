...
try {
    ...
    // Ambil config set_tarif — dua kolom lab yang relevan untuk ranap
    $set = mysqli_fetch_assoc(bukaquery("
        SELECT cara_bayar_lab, kelas_lab
    ...
    if ($set && $reg) {
        if ($set['cara_bayar_lab'] === 'Yes' && !empty($reg['kd_pj'])) {
            $kd_pj = validTeks4($reg['kd_pj'], 20);
            $filter_bayar = "AND kd_pj = '$kd_pj'";
        }
        if ($set['kelas_lab'] === 'Yes' && !empty($reg['kelas'])) {
    ...
    ob_end_clean();
    error_log("get_pemeriksaan_lab_inap.php ERROR: " . $e->getMessage());
    echo json_encode(array(
        'status'  => 'error',
