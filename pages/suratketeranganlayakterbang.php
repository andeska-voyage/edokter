...
$queryCheck = bukaquery("SELECT * FROM surat_keterangan_layak_terbang WHERE no_rawat = '$no_rawat'");
$rsCheck    = mysqli_fetch_array($queryCheck);
$isEdit     = ($rsCheck) ? true : false;

// Generate no_surat otomatis jika baru
$no_surat_default = 'SKLT' . date('YmdHis');
...
<div class="modern-form-container">
    ...
    <div class="form-card">
        <div class="form-content">
            <form id="formLayakTerbang" method="post" action="">
                <!-- Hidden fields -->
                ...
                <div class="section">
                    <div class="section-header">
                        ...
                        <h2>Data Surat Keterangan Layak Terbang</h2>
                    </div>

                    <!-- 3 Kolom: No. Surat | Tanggal | Kehamilan (Minggu) -->
                    <div class="form-grid cols-3">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="tanggal_periksa"
                                   value="<?php echo date('Y-m-d', strtotime($data['tanggal_periksa'])); ?>">
                        </div>
                        <div class="form-group">
                            <label>Kehamilan</label>
                            <div style="display:flex; gap:8px; align-items:center;">
                                <input type="text" name="kehamilan"
                                       value="<?php echo htmlspecialchars($data['kehamilan']); ?>"
                                       maxlength="4" placeholder="0"
                                       style="text-align:center; width:80px; flex-shrink:0;"
                                       oninput="this.value=this.value.replace(/[^0-9]/g,'')">
                                <span style="color:#475569; font-weight:500; white-space:nowrap;">Minggu</span>
                            </div>
                        </div>
                    </div>
        ...
        <div class="action-bar">
            ...
            <?php if($isEdit): ?>
            <button type="button" id="btn-delete" class="btn btn-danger" onclick="confirmDeleteLayakTerbang()">
                <i class="material-icons">delete</i> HAPUS
            </button>
            <?php endif; ?>
            <button type="submit" id="btn-save" form="formLayakTerbang" class="btn btn-primary">
...
<script src="<?php echo BASE_URL; ?>/js/suratketeranganlayakterbang.js?v=<?php echo time(); ?>"></script>