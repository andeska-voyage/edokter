...
$queryCheck = bukaquery("SELECT * FROM surat_bebas_tato WHERE no_rawat = '$no_rawat'");
$rsCheck    = mysqli_fetch_array($queryCheck);
$isEdit     = ($rsCheck) ? true : false;

// Generate no_surat otomatis jika baru
$no_surat_default = 'SBT' . date('YmdHis');
...
<div class="modern-form-container">
    ...
    <div class="form-card">
        <div class="form-content">
            <form id="formKeteranganBebasTato" method="post" action="">
                <!-- Hidden fields -->
                ...
                <div class="section">
                    <div class="section-header">
                        ...
                        <h2>Data Surat Keterangan Bebas Tato</h2>
                    </div>

                    <!-- 4 Kolom: No. Surat | Hasil Pemeriksaan | Keperluan | Tanggal -->
                    <div class="form-grid cols-4">
                        ...
                        <div class="form-group">
                            ...
                            <select name="hasilperiksa">
                                <option value="Bebas Tato" <?php echo ($data['hasilperiksa'] == 'Bebas Tato') ? 'selected' : ''; ?>>Bebas Tato</option>
                                <option value="Bertato"    <?php echo ($data['hasilperiksa'] == 'Bertato')    ? 'selected' : ''; ?>>Bertato</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Keperluan</label>
                            <input type="text" name="keperluan"
                                   value="<?php echo htmlspecialchars($data['keperluan']); ?>"
                                   maxlength="50" placeholder="Melamar pekerjaan, dll">
                        </div>
                        <div class="form-group">
                            <label>Tanggal</label>
                            <input type="date" name="tanggalperiksa"
                                   value="<?php echo date('Y-m-d', strtotime($data['tanggalperiksa'])); ?>">
                        </div>
                    </div>
        ...
        <div class="action-bar">
            ...
            <?php if($isEdit): ?>
            <button type="button" id="btn-delete" class="btn btn-danger" onclick="confirmDeleteBebasTato()">
                <i class="material-icons">delete</i> HAPUS
            </button>
            <?php endif; ?>
            <button type="submit" id="btn-save" form="formKeteranganBebasTato" class="btn btn-primary">
...
<script src="<?php echo BASE_URL; ?>/js/suratketeranganbebastato.js?v=<?php echo time(); ?>"></script>