...
/**
 * get_usg_images.php
 * Handler untuk mengambil gambar USG dari database atau Orthanc
 * 
 * Compatible dengan RME Tab Manager (dipanggil via fetch/AJAX)
 * 
 * Logic:
 * 1. Check database (hasil_pemeriksaan_usg_gambar) dulu
 * 2. Jika kosong, query Orthanc berdasarkan tanggal + no_rkm_medis
 * 3. Return JSON response dengan gambar
 */

// Session: start hanya jika belum aktif (aman untuk RME Tab AJAX context)
...
try {
    // ========================================
    // STEP 1: CHECK DATABASE IMAGES FIRST
    // ========================================
    
    $query_images = "SELECT photo FROM hasil_pemeriksaan_usg_gambar 
    ...
    if ($result_images && mysqli_num_rows($result_images) > 0) {
        while ($row = mysqli_fetch_assoc($result_images)) {
            $photo_url = USG_BASE_URL . $row['photo'];
    ...
    // Jika ada gambar di database, langsung return
    if (!empty($db_images)) {
        echo json_encode([
            'status' => 'success',
            'source' => 'database',
    ...
    // ========================================
    // STEP 2: QUERY ORTHANC IF DATABASE EMPTY
    // ========================================
    
    // Get patient info and exam date
    ...
    // Format tanggal untuk Orthanc (YYYYMMDD)
    $study_date = date('Ymd', strtotime($tanggal));
    
    // Initialize Orthanc API
    $orthanc = ApiOrthanc::fromConfig();
    
    error_log("[USG-IMG] Query Orthanc: NORM=$no_rkm_medis, Date=$study_date (from: $tanggal)");
    
    // Query Orthanc
    $orthanc_series = $orthanc->getAllSeries($no_rkm_medis, $study_date);
    
    error_log("[USG-IMG] Series found: " . count($orthanc_series));
    ...
    // Get thumbnails from Orthanc
    $thumbnails = $orthanc->getThumbnails($no_rkm_medis, $study_date, 20);
    
    error_log("[USG-IMG] Thumbnails count: " . count($thumbnails));
    ...
    error_log("[USG-IMG] ERROR: " . $e->getMessage());
    echo json_encode([
        'status' => 'error',
