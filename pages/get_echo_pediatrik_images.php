...
try {
    // Cek gambar di database lokal
    $ri=bukaquery("SELECT photo FROM hasil_pemeriksaan_echo_pediatrik_gambar WHERE no_rawat='$nrs' ORDER BY photo ASC");
    $db=[];
    if($ri&&mysqli_num_rows($ri)>0){
        while($r=mysqli_fetch_assoc($ri)){
            $db[]=['source'=>'database','data'=>PEMERIKSAAN_ECHO_PEDIATRIK_BASE_URL.$r['photo'],'type'=>'image','photo_path'=>$r['photo']];
        }
    }
    if(!empty($db)){echo json_encode(['status'=>'success','source'=>'database','count'=>count($db),'images'=>$db]);exit();}

    // Fallback ke Orthanc
    $rp=bukaquery("SELECT h.tanggal,r.no_rkm_medis FROM hasil_pemeriksaan_echo_pediatrik h INNER JOIN reg_periksa r ON h.no_rawat=r.no_rawat WHERE h.no_rawat='$nrs' LIMIT 1");
