...
$queryCheck = bukaquery("SELECT * FROM hasil_pemeriksaan_slit_lamp WHERE no_rawat='$no_rawat'");
...
$fields = ['tanggal','kd_dokter','diagnosa_klinis','kiriman_dari','hasil_pemeriksaan'];
...
$images = [];
if($isEdit) {
    $qi = bukaquery("SELECT photo FROM hasil_pemeriksaan_slit_lamp_gambar WHERE no_rawat='$no_rawat'");
...
<div class="modern-form-container">
    <div class="patient-header">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:20px;">
            <div style="display:flex;align-items:center;gap:10px;flex-shrink:0;">
                ...
                <h2 style="margin:0;font-size:15px;font-weight:700;white-space:nowrap;">PEMERIKSAAN SLIT LAMP</h2>
            </div>
    ...
    <div class="form-wrapper">
        <div class="modern-tabs">
            <button class="tab-item active" onclick="switchTabSlitLamp(0)"><i class="material-icons">description</i> Form Pemeriksaan</button>
            <button class="tab-item" onclick="switchTabSlitLamp(1)"><i class="material-icons">image</i> Gambar Slit Lamp
                <span class="tab-badge" id="badge-slitlamp-1" <?php echo count($images)===0?'style="display:none;"':''; ?>><?php echo count($images); ?></span>
            </button>
        </div>

        <form id="formSlitLamp" method="post" action="">
            ...
            <!-- TAB 0: FORM DATA -->
            <div class="tab-content active" id="tab-slitlamp-0">
                <div class="section-card" style="padding:15px;">
                    ...
                    <!-- Hasil Pemeriksaan -->
                    <div class="form-group-modern" style="margin-bottom:0;">
                        <label style="font-size:11px;margin-bottom:3px;">Hasil Periksa</label>
                        <textarea class="form-control-modern" name="hasil_pemeriksaan" rows="10" maxlength="1000"
            ...
            <!-- TAB 1: GAMBAR -->
            <div class="tab-content" id="tab-slitlamp-1">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                    <h3 style="margin:0;"><i class="material-icons">photo_library</i> Gambar Slit Lamp</h3>
                    <div>
                        <input type="file" id="manual-upload-slitlamp" accept="image/*" multiple style="display:none;" onchange="uploadManualSlitLamp(this)">
                        <button type="button" onclick="document.getElementById('manual-upload-slitlamp').click()"
                ...
                <div id="image-gallery-slitlamp"><div class="loading-spinner"><i class="material-icons">info</i> Klik tab untuk memuat gambar...</div></div>
            </div>
            ...
            <div class="action-buttons">
                <div style="display:flex;align-items:center;gap:15px;">
                    ...
                    <span style="font-size:12px;color:#666;font-weight:600;">Tab <span id="current-tab-number-slitlamp">1</span> dari 2</span>
                </div>
                <div style="display:flex;gap:10px;">
                    <button type="button" class="btn-modern btn-secondary-modern" onclick="kembaliSlitLamp()"><i class="material-icons" style="font-size:16px;">arrow_back</i> KEMBALI</button>
                    <button type="button" class="btn-modern btn-secondary-modern" id="btn-prev-slitlamp" onclick="previousTabSlitLamp()" style="display:none;"><i class="material-icons" style="font-size:16px;">navigate_before</i> SEBELUMNYA</button>
                    <button type="button" class="btn-modern btn-primary-modern" id="btn-next-slitlamp" onclick="nextTabSlitLamp()">SELANJUTNYA <i class="material-icons" style="font-size:16px;">navigate_next</i></button>
                    <button type="submit" class="btn-modern btn-primary-modern" id="btn-save-slitlamp" style="display:none;"><i class="material-icons" style="font-size:16px;">save</i> SIMPAN DATA</button>
                    <button type="button" class="btn-modern btn-danger-modern" id="btn-delete-slitlamp" style="display:none;" onclick="confirmDeleteSlitLamp()" <?php echo !$isEdit?'disabled':''; ?>><i class="material-icons" style="font-size:16px;">delete</i> HAPUS DATA</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <!-- Modal Gambar -->
    <div id="imageModalSlitLamp" style="display:none;position:fixed;z-index:9999;left:0;top:0;width:100%;height:100%;background:rgba(0,0,0,0.9);" onclick="closeImageModalSlitLamp()">
        <span style="position:absolute;top:15px;right:35px;color:#f1f1f1;font-size:40px;font-weight:bold;cursor:pointer;">&times;</span>
        <img id="modalImageSlitLamp" style="margin:auto;display:block;max-width:90%;max-height:90%;object-fit:contain;margin-top:50px;">
    </div>
</div>
...
<script>
(function(){
    if(window._slitLampJsLoaded){if(typeof window._slitLampInit==='function')setTimeout(window._slitLampInit,300);return;}
    var s=document.createElement('script');s.src='<?php echo BASE_URL; ?>/js/pemeriksaanslitlamp.js?v=<?php echo time(); ?>';
    s.onload=function(){window._slitLampJsLoaded=true;if(typeof window._slitLampInit==='function')setTimeout(window._slitLampInit,100);};
